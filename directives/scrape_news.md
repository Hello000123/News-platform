...
- **Recurring coupon headlines are not one news story** — affiliate pages such as “Motley Fool Promo Code,” “Bose Promo Code,” and “Hoka Coupon Codes” can share discount amounts, “off,” month names, and years while covering unrelated merchants. Before ordinary title similarity, extract and compare the merchant text that precedes recurring `promo code`, `coupon code`, `discount code`, or plural `coupons` templates. Different non-empty merchant subjects must never cluster; alternate pages for the same merchant may still group. This prevents a high-report-count affiliate cluster from entering the Top 5 with tens of unrelated sources and overwhelming rewrite fidelity checks.
...
- **Top 5 bounded recovery, atomicity, and idempotency** — each story may make one fresh request after an explicitly retryable API/model/validation failure. Reuse the same `debugBatchId`. Commit the canonical rewrite, every selected duplicate merge, and the authoritative user/batch/article idempotency record in one D1 transaction; if any selected article changed while the model was running, commit none of them. If the first request committed but its HTTP response was lost, return the already-saved rewrite from that idempotency record without charging another model request or overwriting the draft. Debug logging remains best-effort diagnostics and must never be required for correctness. After the bounded model correction, a headline-bounded pipeline draft may deterministically replace a one-to-one modified or punctuation-changed quotation with the exact source quotation while leaving optional omitted quotations omitted; rerun every language, term, numeric, format, quotation-attribution, and invented-quotation check afterward, and never repair split, merged, invented, or misattributed quotations this way. After both focused model corrections, the draft may also omit an optional body sentence only when unsupported numbers are the sole remaining failure, the headline is already safe, and at least two complete safe body sentences remain; rerun all checks afterward. Never remove or alter an unsafe headline automatically.
- **Chinese semicolon house style** — rewrite headlines and narration must not use the Chinese semicolon `；`; instruct the model to choose `，` or `。` according to the sentence, then deterministically convert any remaining narrative `；` to `，`. Never normalize punctuation inside quoted source text, because direct quotations must remain exact.
...
- **Cloudflare Workers DNS validation and subrequests** — Workers does not implement `node:dns` `lookup()`. The web feed fetcher must use both `resolve4()` and `resolve6()` so its SSRF guard checks every returned address and every redirect destination. Workerd returns CNAME labels in these arrays alongside terminal A/AAAA addresses for some publishers; discard non-IP labels before applying the public-address test, but continue rejecting any returned IP that is not public. DNS queries, feed fetches, and D1 operations all count as Worker subrequests. Keep the explicit `wrangler.jsonc` ceiling at 1,000 for the current 18-feed batch, below the Workers Paid default of 10,000 and aligned with D1's documented Paid-plan query ceiling per invocation. Cloudflare does not bill Worker subrequests separately.
