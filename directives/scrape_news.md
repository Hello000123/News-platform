...
- **Empty or stale page selectors** — never overwrite a usable feed title, body, author, date, or image with an empty page-scrape value. Records without an ID, title, URL, or non-empty article text are logged and skipped before export, preventing one broken source row from invalidating the whole UI import.
